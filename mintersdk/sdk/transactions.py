...
class MinterTx(object):
    ...
    def generate_tx_rlp(self):
        ...
        tx_struct: dict = self._structure_from_instance()
    ...
    def from_raw(cls, raw_tx):
        ...
        elif struct['type'] == MinterBuySwapPoolTx.TYPE:
            _class = MinterBuySwapPoolTx
        elif struct['type'] == MinterSellSwapPoolTx.TYPE:
            _class = MinterSellSwapPoolTx
        elif struct['type'] == MinterSellAllSwapPoolTx.TYPE:
            _class = MinterSellAllSwapPoolTx
        elif struct['type'] == MinterAddLiquidityTx.TYPE:
            _class = MinterAddLiquidityTx
        elif struct['type'] == MinterRemoveLiquidityTx.TYPE:
            _class = MinterRemoveLiquidityTx
        else:
...
class MinterSellSwapPoolTx(MinterTx):
    """ Sell Swap Pool transaction """
    ...
    def __init__(self, coins, value_to_sell,
    ...
    def _structure_from_instance(self):
        ...
        struct.update({
            'type': self.TYPE,
            'data': {
                'coins': [id if id else '' for id in self.coins],
    ...
    def _structure_to_kwargs(cls, structure):
        ...
        kwargs['data'].update({
            'coins': [
                int.from_bytes(w, 'big') for w in kwargs['data']['coins']
            ],
...
class MinterBuySwapPoolTx(MinterTx):
    """ Buy Swap Pool transaction """

    # Type of transaction       0x18
    TYPE = 24

    # Fee units
    COMMISSION = 100

    def __init__(self, coins, value_to_buy,
    ...
    def _data_from_raw(cls, raw_data):
        ...
        return {
            ...
            'max_value_to_sell': raw_data[2]
        }


class MinterSellAllSwapPoolTx(MinterTx):
    """ Buy Swap Pool transaction """
    ...
    def _data_from_raw(cls, raw_data):
        ...
        return {
            'coins': raw_data[0],
            'min_value_to_buy': raw_data[1]
        }


class MinterAddLiquidityTx(MinterTx):
    """ Add liquidity transaction """

    # Type of transaction 0x15
    TYPE = 21

    # Fee units
    COMMISSION = 100

    def __init__(self, coin0, coin1, volume0, maximumVolume1, **kwargs):
        """
        Args:
            Coin0          uin32
            Coin1          uin32
        ...
        self.maximumVolume1 = maximumVolume1
    ...
    def _structure_from_instance(self):
        ...
        struct.update({
            'type': self.TYPE,
            'data': {
                'coin0': int(self.coin0),
                'coin1': int(self.coin1),
                'volume0': MinterHelper.to_pip(self.volume0),
                'maximumVolume1': MinterHelper.to_pip(self.maximumVolume1)
            }
        })
    ...
    def _structure_to_kwargs(cls, structure):
        ...
        # Data will be passed as additional kwarg
        kwargs['data'].update({
            'coin0': kwargs['data']['coin0'],
            'coin1': kwargs['data']['coin1'],
            'volume0': MinterHelper.to_bip(
                int.from_bytes(kwargs['data']['volume0'], 'big')
            ),
            'maximumVolume1': MinterHelper.to_bip(
                int.from_bytes(kwargs['data']['maximumVolume1'], 'big')
            )
        })
    ...
    def _data_from_raw(cls, raw_data):
        """ Parent method implementation """
        return {
            'coin0': int.from_bytes(raw_data[0], 'big'),
            'coin1': int.from_bytes(raw_data[1], 'big'),
            'volume0': raw_data[2],
            'maximumVolume1': raw_data[3],
        }


class MinterRemoveLiquidityTx(MinterTx):
    """ Remove liquidity transaction """

    # Type of transaction 0x16
    TYPE = 22

    # Fee units
    COMMISSION = 100

    def __init__(self, coin0, coin1, liquidity, minimumVolume0, minimumVolume1, **kwargs):
        """
        Args:
            Coin0          uin32
            Coin1          uin32
        ...
        self.minimumVolume0 = minimumVolume0
        self.minimumVolume1 = minimumVolume1
    ...
    def _structure_from_instance(self):
        ...
        struct.update({
            ...
            'data': {
                'coin0': int(self.coin0),
                'coin1': int(self.coin1),
                'liquidity': MinterHelper.to_pip(self.liquidity),
                'minimumVolume0': MinterHelper.to_pip(self.minimumVolume0),
                'maximumVolume1': MinterHelper.to_pip(self.minimumVolume1)
            }
        })
    ...
    def _structure_to_kwargs(cls, structure):
        ...
        kwargs['data'].update({
            'coin0': kwargs['data']['coin0'],
            'coin1': kwargs['data']['coin1'],
            'liquidity': MinterHelper.to_bip(
                int.from_bytes(kwargs['data']['liquidity'], 'big')
            ),
            'minimumVolume0': MinterHelper.to_bip(
                int.from_bytes(kwargs['data']['minimumVolume0'], 'big')
            ),
            'minimumVolume1': MinterHelper.to_bip(
                int.from_bytes(kwargs['data']['minimumVolume1'], 'big')
            )
        })
    ...
    def _data_from_raw(cls, raw_data):
        ...
        return {
            'coin0': int.from_bytes(raw_data[0], 'big'),
            'coin1': int.from_bytes(raw_data[1], 'big'),
            'liquidity': raw_data[2],
            'minimumVolume0': raw_data[3],
            'minimumVolume1': raw_data[4],
        }
